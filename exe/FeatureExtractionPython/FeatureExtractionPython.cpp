...
// https://stackoverflow.com/questions/41240610/how-to-call-python-from-a-boost-thread
...
struct OpenfaceFrame {
	...
	void operator = ( const OpenfaceFrame& src ) {
		...
		head_rot = src.head_rot;
		head_pos = src.head_pos;
		landmarks.reserve(src.landmarks.size());
		copy(src.landmarks.begin(), src.landmarks.end(), landmarks.begin());
	}
	
	boost::python::dict to_python() {
		
		boost::python::dict pyframe;
		
		boost::python::list g0;
		boost::python::list g1;
		boost::python::list hr;
		boost::python::list hp;
		...
		g1.append( gaze_1.y );
		g1.append( gaze_1.z );
		hr.append( head_rot.x );
		hr.append( head_rot.y );
		hr.append( head_rot.z );
		hp.append( head_pos.x );
		hp.append( head_pos.y );
		hp.append( head_pos.z );
		for ( int i = 0; i < LANDMARKS_NUM; ++i ) {
			boost::python::list vs;
			vs.append( landmarks[i].x );
			vs.append( landmarks[i].y );
			vs.append( landmarks[i].z );
			ls.append( vs );
		}
...
public:
	...
	OpenfaceVideo() : pycall(0) {}
	
	OpenfaceVideo( const OpenfaceVideo& src ) : pycall(0) {
	...
	std::string get_AU() {
		return get_argument( "-au" );
	}
	
	void set_AU( const char* path ) {
		append_argument( "-au", std::string( path ), true );
	}
	...
	bool start() {
		...
		// quite important to set to true!
		_running = true;
		// Load the modules that are being used for tracking and face analysis
		// Load face landmark detector
		LandmarkDetector::FaceModelParameters det_parameters(arguments);
		...
		while ( _running ) {
			...
			while (!captured_image.empty()) {
				...
				char character_press = visualizer.ShowObservation();
				// quit processing the current sequence (useful when in Webcam mode)
				if ( !_running || character_press == 'q') {
					break;
				}
				// Setting up the recorder output
				// The number of channels in HOG is fixed at the moment, as using FHOG
				open_face_rec.SetObservationHOG(detection_success, hog_descriptor, num_hog_rows, num_hog_cols, 31);
				open_face_rec.SetObservationVisualization(visualizer.GetVisImage());
				open_face_rec.SetObservationActionUnits(face_analyser.GetCurrentAUsReg(), face_analyser.GetCurrentAUsClass());
				open_face_rec.SetObservationLandmarks(face_model.detected_landmarks, face_model.GetShape(sequence_reader.fx, sequence_reader.fy, sequence_reader.cx, sequence_reader.cy),
					face_model.params_global, face_model.params_local, face_model.detection_certainty, detection_success);
	...
	void new_frame( Utilities::RecorderOpenFace& rec ) {
		...
		// if a callback function is defined, updating the local copy and passing it
		// to the callback function
		if ( pycall != 0 ) {
			pyframe = frame.to_python();
			pycall->call( pyframe );
		}
	...
	void callback_frame( const boost::python::object& object ) {
		if ( pycall != 0 ) {
			delete pycall;
		}
		pycall = new py_callable{object};
	}
...
BOOST_PYTHON_MODULE(PyOpenfaceVideo) {
	...
	*/
	...
	class_<OpenfaceVideo>("OpenfaceVideo")
    	...
    	.add_property("AU", &OpenfaceVideo::get_AU, &OpenfaceVideo::set_AU)
    	.def("start", &OpenfaceVideo::start)
    	.def("stop", &OpenfaceVideo::stop)
    	.def("is_running", &OpenfaceVideo::is_running)
		.def("callback_frame",&OpenfaceVideo::callback_frame);
