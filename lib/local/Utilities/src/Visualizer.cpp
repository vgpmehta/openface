...
const std::map<std::string, std::string> AUS_NAMES = {
	...
	const std::vector<std::pair<std::string, double> >& au_occurences)
{
    ...
    size_t idx = 0;
    for (auto& kv : au_intensities) {
            bool present = (au_occurences[idx].second != 0);
            auto offset = MARGIN_Y + idx * (AU_TRACKBAR_HEIGHT + 10);
            std::ostringstream au_i;
            au_i << std::setprecision(2) << std::setw(4) << std::fixed << kv.second;
            cv::putText(action_units_image, kv.first, cv::Point(10, offset + 10), CV_FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(present ? 0 : 200, 0, 0), 1, CV_AA);
            cv::putText(action_units_image, AUS_NAMES.at(kv.first), cv::Point(55, offset + 10), CV_FONT_HERSHEY_SIMPLEX, 0.3, CV_RGB(0, 0, 0), 1, CV_AA);
            ...
            if(present) {
                ...
                cv::rectangle(action_units_image, cv::Point(MARGIN_X, offset),
                                              cv::Point(MARGIN_X + AU_TRACKBAR_LENGTH * kv.second/5, offset + AU_TRACKBAR_HEIGHT),
